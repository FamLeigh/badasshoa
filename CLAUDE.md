...
- **New `/admin/reset-demo.php`** (super_admin only). Wipes every association-scoped row in the demo target (assoc #2 — gated by `ALLOWED_TARGET_IDS = [2]` const at the top of the file) and re-clones from Bellair (#1) with all user first/last names randomized, emails replaced with `demo+<hex>@badasshoa.com`, phones replaced with `555-01xx`. Operator types a demo password on the form which is applied to every cloned user so anyone can be logged in as for demo purposes. Also copies `storage/uploads/1/` → `storage/uploads/2/` so demo files are independent of Bellair's.
...
- **Safety rails:** target ID must be in `ALLOWED_TARGET_IDS`, super_admin-only via `_bootstrap.php`, confirm-by-typing target slug, CSRF token, single DB transaction with rollback on any error, file copy staged in `storage/uploads/2.new/` and only swapped in on commit. Audit-logged as `demo.reset` with row counts.
...
- **Local testing limited** — local DB has only assoc #1 + #3, no #2, so the GET confirm screen 404s locally. Validated via `php -l` and code review. First real test happens on prod after deploy; recommend running it once with a throwaway demo password to confirm row counts before relying on the output.
